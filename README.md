...
- 박진영: `/health` 의 `env` 가 배포에서도 `local` 로 나온다.
  `docker-compose.yml` 의 `environment: APP_ENV: local` 이 초기 스켈레톤부터
  하드코딩돼 있고, compose 의 `environment` 가 `env_file`(.env) 를 덮으므로
  **`.env` 에 `APP_ENV=prod` 를 넣어도 바뀌지 않는다.** 지금은 `/health` 표시
  외에 쓰이는 곳이 없어 동작에는 영향이 없지만, 라이브 확인 지표로 쓸 수 없다.
