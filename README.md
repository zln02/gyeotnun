...
새로 합류하거나 새 세션을 시작한다면 [`CLAUDE.md`](CLAUDE.md) 를 먼저 읽는다 —
되돌릴 수 없는 실수를 낸 지점들(운영 DB 삭제·로컬 우회·배포 절차)이 거기 모여 있다.
...
- **확인 기록의 보관 기간은 90일**입니다. `checks` · `evidence` · `taggings` ·
  `events` · `error_logs` · `judgment_logs` 를 매일 04:00 자동 삭제합니다
...
### 판단 행동 로그 (2026-08-20 추가)

- 목적: `events` 가 **화면을 어떻게 썼나**를 낱개로 쌓는다면, 이쪽은 **판단을 어떻게
  했나**를 세션 1개당 1행으로 남깁니다 → `judgment_logs` 테이블.
  질문을 열었는지(성급 판단의 역지표), 무엇을 확인했는지(출처·보낸 곳·날짜·조건),
  무엇을 골랐고 얼마나 걸렸는지, 훈련 카드를 맞혔는지.
  `session_type`(baseline / training / posttest)으로 나누면 훈련 전후 비교가 됩니다.
- **수집 항목**: 전부 참/거짓·숫자이거나 **미리 정한 짧은 코드값**입니다.
  자유 텍스트가 들어갈 컬럼이 **하나도 없습니다** — 허용 목록에 없는 값은 잘라 넣지
  않고 **버립니다**(`services/judgment_log.py`). 이 두 가지는
  `api/tests/test_judgment_log.py` 가 자동으로 지키고, 가드 자신이 실패하는지도
  같은 파일에서 시험합니다.
- `user_ref` 는 다른 테이블과 동일하게 **SHA-256 해시**입니다(`check_store` 와 같은 값).
- ★ **NULL 은 '아니오'가 아니라 '측정하지 않음'입니다.** 화면이 보고하지 않은 확인
  항목은 `False`(=확인 안 함)가 아니라 NULL 로 남고, 집계는 NULL 을 표본에서 뺍니다.
  모든 비율에 **표본 수를 함께** 반환합니다 — "3명 중 1명"과 "300명 중 100명"이
  똑같이 0.333 으로 보이지 않도록.
- 계측은 `events` 와 같은 **fire-and-forget** 입니다. 실패해도 확인 화면을 막지 않고
  서버만 `ST-003` 으로 인지합니다.
- 기록 지점: `POST /checks`(세션 시작) · `POST /checks/{id}/dialogue`(질문 노출) ·
  `POST /checks/{id}/verdict`(판단 확정) · `POST /training/result`(훈련 결과, 신설).
- 집계: `GET /api/v1/judgments/summary` — ★ **운영자 전용**(`X-Admin-Token`).
  `events/summary`·`errors/summary` 와 같은 게이트입니다.
...
| `judgment_logs` | 판단 행동 1세션(2026-08-20) - 무엇을 확인하고 무엇을 골랐나. **자유 텍스트 컬럼 없음**, device 해시만 |
...
지웠다 — 결과는 위 [8. DB 스키마](#8-db-스키마-10테이블) 의 코퍼스 규모에 있다.
