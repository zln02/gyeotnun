...
const CASES = [
  ...
  },
  {
    tier: 'tier-warn',
    label: '의심',
    // ★ 이전엔 '복지로 노후준비서비스...' 문구를 썼는데, match_scam_cases() 의
    //   불용어/가중치 채점 수정(docs/evaluation/eval_30_report.md) 이후 그 문구는
    //   더 이상 사기 사례와 오매칭되지 않는다(정상 안내문 오판 제거가 의도한 결과) -
    //   그래서 실제로 유사 사기 사례와 매칭되는 문구(평가세트 S10)로 교체했다.
    text: '금융감독원 보호조치입니다. 예금을 모두 지정 계좌로 옮겨야 합니다.',
  },
  {
    tier: 'tier-unknown',
    label: '확인 불가',
