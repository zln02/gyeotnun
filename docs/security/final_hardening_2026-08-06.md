...
## 2. create_check 의 device_id="anonymous"

**거부**를 택했다. `require_owner` 에서 요청자 또는 소유자가 빈값/"anonymous"면
거부한다(`_NON_OWNER_IDS`). 정상 프론트는 항상 실제 UUID(`deviceId()`)를 보내므로
정상 흐름을 깨지 않는다(실측 확인: UUID 소유자 200, 익명 조회 404).

- 예전: device_id 생략 → 소유자 "anonymous" 로 저장 → "anonymous" 보내는 누구나 상호 조회.
- 지금: 그렇게 만들어진 기록은 아무도 못 읽는다(안전한 실패). 이미 저장된 익명 기록도 무력화.
...
- 다중 워커 → `_MEMORY_STORE` 를 DB/Redis 로 이관(그때 checks 도 보관 잡 대상에 추가).
- `device_id` 위조 가능성 → 서버 발급 세션 등 근본 인증.
- AWS 보안그룹에서 5432/기타 인바운드 콘솔 점검(현재 바인딩상 외부 도달은 불가).
