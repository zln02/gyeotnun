...
ERROR_CODES: List[ErrorCodeDef] = [
    ...
    },
    {
        "code": "ST-003",
        "area": "저장",
        "situation": "사용자 행동 계측(이벤트·판단로그) 1건을 DB에 저장하는 데 실패함",
        "user_message": "",
        ...
        "internal_handling": "routers/events.py DB insert 실패 시 그대로 200(accepted=0) 을 돌려주고"
                              ...
                              "★ 2026-08-20: services/judgment_log.py(판단 행동 로그)도 같은 코드를 쓴다 - "
                              "같은 부류(관측용 부가 기록)이고 대응도 같기 때문이다.",
    },
    # ══════════════════════════════════════════════════════════ 외부연동 (EX)
    {
        "code": "EX-001",
    ...
    },
    {
        "code": "EX-005",
        "area": "외부연동",
        "situation": "요청이 제한 시간(30초) 안에 끝나지 않아 화면에서 끊음",
        # ★ 이 문구는 web/src/api.js 의 safeFetch() 가 실제로 띄우는 문장과 글자까지 같아야
        #   한다. 다르면 화면과 이 표가 서로 다른 안내를 하게 되고, 그 순간 '단일 소스'가
        #   깨진다(그래서 EX-004 와 마찬가지로 프론트 발생 코드도 여기 등록한다).
        "user_message": "시간이 오래 걸리고 있어요. 다시 해보시겠어요?",
        "recovery": "'다시 시도'를 눌러 한 번 더 요청합니다. 반복되면 잠시 뒤에 다시 시도합니다.",
        "internal_handling": "프론트가 AbortController 로 REQUEST_TIMEOUT_MS(30초)에 요청을 끊고 이 코드로 "
                              "변환한다(web/src/api.js safeFetch). 서버는 응답을 만들다 끊긴 것이라 이 코드를 "
                              "직접 발생시키지 않는다 - EX-004(요청 미도달)와 달리 요청 자체는 서버에 닿았다. "
                              "30초는 가장 느린 정상 경로(질문 생성 최대 약 14초)의 2배 여유이고, "
                              "nginx proxy_read_timeout(120초)보다 짧아 화면이 먼저 끊는다.",
    },
    {
        "code": "EX-006",
        "area": "외부연동",
        "situation": "최근 근거 검색 중 BM25 폴백으로 전환된 비율이 임계(50%)를 넘음 "
                     "- 개별 실패가 아니라 '반복되고 있다'는 상태 경고",
        "user_message": "",
        "recovery": "사용자 조치 불필요. 팀이 임베딩 인덱스·모델 로드 상태를 점검한다.",
        "internal_handling": "services/search.py 가 최근 N건의 폴백 여부를 세어 임계를 넘으면 1회 "
                              "경고한다(동작 변경 없음, 관측만). 2026-08-13 추가 - EX-003 은 개별 "
                              "발생만 남기고 '계속 이러고 있다'를 말해 주지 않는다. 폴백 중에는 "
                              "코퍼스 크기에 따라 실질적으로 느슨해지는 BM25 하한선으로 근거가 붙는데 "
                              "화면은 평소와 같아서, 장애를 아무도 모른 채 지나갈 수 있다 "
                              "(질문 생성 쪽 GN-003 과 같은 형식·같은 이유).",
    },
    {
        "code": "EX-007",
        "area": "외부연동",
        "situation": "문자 속 주소의 최종 연결지를 확인하지 못함 "
                     "(타임아웃·홉 초과·HEAD 거부·사설 IP 차단·DNS 실패)",
        # ★ 빈 문자열이다. 실패해도 사용자에게 아무 말도 하지 않는다.
        #   "확인하지 못했습니다"라고 쓰면 어르신은 그것을 "수상하다"로 읽는다.
        #   실제로 검출한 것이 없는데 불안을 만드는 것은 곁눈이 막으려는 짓이다.
        "user_message": "",
        "recovery": "사용자 조치 불필요 - 주소 줄만 화면에서 빠지고 확인 흐름은 그대로 진행된다.",
        "internal_handling": "services/url_expand.py 가 HEAD 전용·홉 상한 3·타임아웃 3초로 "
                              "주소를 펼친다. 실패하면 주소 줄을 생략하고 나머지 흐름은 그대로 "
                              "간다(자동 폴백이라 화면이 평소와 같다 - GN-002·ST-003 과 같은 부류). "
                              "★ detail 에 사유를 남기는데 private_ip 로 멈춘 건은 따로 구분한다 - "
                              "단순 실패가 아니라 누군가 우리 서버를 내부망 탐색에 쓰려 한 흔적일 "
                              "수 있다. 2026-08-15 추가, 설계: docs/evaluation/URL펼치기_설계_2026-08-15.md",
    },
    # ══════════════════════════════════════════════════════════ 공통(미분류)
    {
        "code": "SYS-000",
