...
★ 운영 경로 미연결 · 측정 전용 (레거시 아님)
  routers/ 어디에서도 부르지 않는다. 질문이 "입력에 없는 것을 전제"하거나
  "입력에 있는 것을 없다고 전제"하는지를 **세기 위한** 검사기이고, 지금은
  experiments/exp_question_premise.py · exp_na_rate_org_cases.py 와 테스트에서만 쓴다.
  운영 연결 여부는 미결이다 - docs/evaluation/본체결함_발견대장.md M1 참고.
  ★ 연결하려면 prompt_chain.validate_question 과의 관계를 먼저 정해야 한다
    (둘 다 질문을 막을 수 있어, 순서와 책임이 겹친다).
