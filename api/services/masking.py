...
구현 상태
  - 텍스트 마스킹(개인 연락처/계좌/주민번호/카드): 실제 동작 (아래 정규식)
    ★ '개인 연락처' 다. 휴대전화(01x)·인터넷전화(070)·유선 지역번호(02~06x)를 가린다.
      **기관 대표번호(1577·1588 등)는 일부러 가리지 않는다** - 화면이 "기관 대표번호로
      직접 전화해 확인해 보세요"라고 권하는데 그 번호를 가리면 우리가 권한 확인
      방법을 막는 꼴이 된다. 국제표기(+82 2-…)·수신거부번호(0808…)도 같은 이유다.
      ★ 결함이 아니라 의도된 경계다(2026-08-16 실사용 점검에서 확인·명문화).
        PHONE_PREFIX_PATTERN 을 넓히기 전에 이 문단을 먼저 읽을 것.
