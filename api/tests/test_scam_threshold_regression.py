...
■ ★ 이 테스트는 하한선 결정의 '전제'다 (2026-08-13)
  docs/evaluation/하한선_결정_2026-08-13.md 에서 min_score 를 5.0 으로 유지하기로
  결정했고, 그 근거의 하나가 "남는 정상 오판이 각 세트 1건뿐이라 감당할 수 있다"
  였다. 즉 **1건이 늘어나는 순간 그 결정의 전제가 깨진다.** 이 테스트가 그 경보다.
  실패하면 상수를 고치기 전에 위 결정 문서부터 다시 읽을 것.
"""
