 ...
 *
 * ★ 상태는 5개다 (verdict.js 참고) - danger / warn / act / hold / ok
 *   Figma 원본에는 빨간 경고 한 벌만 그려져 있는데, 그대로 두면 공식 자료를
 *   잘 찾은 정상 안내문에도 빨간 경고가 나간다. 레이아웃·구성은 그대로 두고
 *   색·제목·'사실 한 줄'만 상태별로 바꾼다.
 *
 *   ★ 2026-08-13 에 act(행동 프레임)가 늘어 4개 → 5개가 됐다. 아래 HERO 가
 *     이미 5개를 쓰고 있었는데 이 주석만 4개로 남아 있었다(2026-08-14 감사).
 *
 * ★★ Figma 에 있지만 넣지 않은 것 ★★
...
export default function Judgment({ evidence, checkData, onStart }) {
  ...
  return (
    <div className="judgment">
      ...
      {/* ②-보조 근거 유보 한 줄 (2026-08-17 채택).
          ★ factBlock 은 한 줄만 고른다. 의심 신호가 있으면 그쪽이 이기고
            "같은 안내인지는 확인하지 못했어요" 가 사라진다 — **하필 사칭 건에서.**
            참고자료에는 다른 제도의 공식 문서가 그대로 보이는데 유보만 없어졌다.
          ★ 이 줄은 '글'이 아니라 **우리 근거**에 대한 유보다. 그래서 정상 문자에
            붙어도 참이고 해롭지 않다(실측 142건 중 정상 2건 포함).
          ★ tier 색을 쓰지 않는다 - 판정이 아니다. */}
      {s.factNote ? <p className="judgment-fact-note">{s.factNote}</p> : null}
